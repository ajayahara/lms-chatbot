...
const ChatHistory : React.FC<ChatHistoryProps> = ({toggleChatTab}) => {
  ...
  return (
    <div>
      {/* New Chat Card */}
      <div className='flex justify-between items-center px-2 shadow-lg cursor-pointer' onClick={() => toggleChatTab(null)}>
        <div>
      ...
      <div>
        {
          allInteractions && allInteractions.map((interaction) => (
            <div className='mx-2 my-5 shadow-md p-2 cursor-pointer flex justify-between items-center' onClick={() => toggleChatTab(interaction.id)} key={interaction.id}>
              <div>
                {interaction.title.slice(0, 30)}...
              </div>
              <span>{interaction.ended_at?.split("T")[1]}</span>
              <div>
                  <IoIosArrowForward/>
              </div>
              </div>
